...
처음 사는 S&amp;P500 ETF는 몇 주부터 시작하면 좋을까? &nbsp; 처음이라면 1주로 충분하다. 중요한 것은 큰돈을 넣는 것이 아니라 매수, 체결, 평가손익, 분배금, 세금 안내를 직접 확인하는 경험이다. 1주를 사보고 상품 구조를 이해한 뒤에 적립식 투자 여부를 정해도 늦지 않다.`}).add({id:13,href:"/posts/sp500-vs-schd-which-to-buy/",title:"S&P500 vs SCHD, 뭐 사야 할까? 수익률부터...",description:"S&P500과 SCHD 중 무엇을 사야 할지 고민이라면 이 글 하나로 끝내자. 수익률, 배당, 투자 성향별 추천까지 초보자 기준으로 쉽게 정리했다.",content:`S&amp;P500 vs SCHD, 처음 투자 시작할 때 거의 무조건 마주치는 고민이다.
...
Q3. 둘 중 하나만 꼭 골라야 한다면? &nbsp; 초보자 기준에서는 S&amp;P500을 먼저 선택하는 게 더 합리적이다. 이후 상황에 따라 SCHD를 추가하면 된다.`}).add({id:14,href:"/posts/vix-index/",title:"VIX 지수란? 공포지수로 보는 주식시장 변동성과 투자...",description:"VIX 지수의 뜻과 계산 원리, S&P500과의 관계, 투자자가 활용하는 방법, VIX 선물과 ETF 투자 시 주의할 점을 쉽게 정리했다.",content:`주식시장이 크게 흔들릴 때 뉴스에서 자주 나오는 말이 있다. 바로 VIX 지수다. &ldquo;공포지수가 급등했다&rdquo;, &ldquo;VIX가 안정되면서 증시가 반등했다&rdquo; 같은 표현을 한 번쯤 봤을 것이다.
...
VIX ETF를 장기 보유해도 될까? &nbsp; 주의가 필요하다. VIX ETF나 ETN은 대부분 VIX 선물을 활용하기 때문에 장기 보유 수익률이 VIX 지수와 다르게 움직일 수 있다. 선물 만기 교체 비용과 큰 변동성 때문에 단기 대응 상품에 가까운 경우가 많다.`}).add({id:15,href:"/posts/sole-proprietor-vs-corporation/",title:"개인사업자와 법인사업자 차이, 창업 전에 꼭 알아야 할 기준",description:"창업할 때 개인사업자와 법인사업자 중 무엇을 선택해야 할지 설립 절차, 세금, 책임 범위, 자금 조달, 운영 방식 차이로 쉽게 정리했다.",content:`창업을 준비하다 보면 생각보다 빨리 마주치는 질문이 있다. &ldquo;개인사업자로 시작할까, 법인사업자로 시작할까?&ldquo;라는 문제다. 아이템을 정하고, 상호를 고민하고, 사업장을 알아보는 일도 중요하지만 사업 형태를 어떻게 정하느냐에 따라 세금, 책임, 회계 처리, 투자 유치 방식이 달라진다.
...
경기지역화폐 카드를 받기 전에도 충전할 수 있을까? &nbsp; 지역과 카드 상태에 따라 앱에서 충전 메뉴가 보일 수 있지만, 실제 결제는 카드 등록 이후 가능한 경우가 많다. 처음 신청했다면 카드를 수령한 뒤 앱에 등록하고 계좌 연결까지 마친 다음 충전하는 흐름으로 보는 것이 안전하다.`}).add({id:17,href:"/posts/how-to-study-economics/",title:"경제 공부하는 방법, 금융 문맹을 벗어나기 위한 현실적...",description:"경제 공부를 어디서부터 시작해야 할지 모르는 초보자를 위해 기본 용어, 경제 뉴스 읽기, 책 선택, 가계부와 소액 투자까지 현실적인 공부 방법을 정리했다.",content:`경제 공부를 해야겠다고 마음먹는 순간은 대개 불편함에서 온다. 월급은 들어오는데 돈이 어디로 나가는지 모르겠고, 금리가 오른다는데 내 대출이 왜 부담스러워지는지 모르겠고, 뉴스에서는 환율과 물가 이야기가 계속 나오는데 내 생활과 어떻게 연결되는지 감이 잡히지 않는다.
...
위기 때 주식은 모두 팔아야 할까? &nbsp; 모든 주식을 감정적으로 매도하는 것은 위험할 수 있다. 당장 쓸 돈으로 투자했다면 줄이는 것이 맞지만, 장기 자금이라면 비상금과 부채 상황을 먼저 확인한 뒤 분할 매수나 리밸런싱 기준에 따라 판단하는 것이 낫다.`}).add({id:20,href:"/posts/spillover-effect/",title:"경제용어: 스필오버 효과, 경제에서 좋은 영향과 나쁜 ...",description:"스필오버 효과의 뜻과 경제, 관광, 조직관리, 상권에서 나타나는 긍정적 영향과 부정적 영향을 초보자도 이해하기 쉽게 정리했다.",content:`경제 기사를 읽다 보면 &ldquo;스필오버 효과&quot;라는 표현이 종종 나온다. 영어로는 Spillover Effect라고 하고, 말 그대로 어떤 영향이 한곳에만 머무르지 않고 주변으로 흘러넘치는 현상을 뜻한다.
...
투자할 때도 스필오버 효과를 볼 수 있을까? &nbsp; 볼 수 있다. 특정 산업의 대규모 투자가 장비, 소재, 부품, 물류 기업으로 번질 수 있고, 한 나라의 금리 변화가 다른 나라 주식시장과 환율에 영향을 줄 수도 있다. 투자자는 직접 수혜주뿐 아니라 주변으로 번지는 영향을 함께 살펴볼 필요가 있다.`}).add({id:21,href:"/posts/financial-freedom-roadmap/",title:"경제적 자유, 누구나 이룰 수 있는 실천 로드맵",description:"경제적 자유를 막연한 꿈이 아니라 실천 가능한 목표로 만들기 위해 지출 관리, 비상금, 부채 정리, 투자, 부수입 로드맵을 정리했다.",content:`경제적 자유라는 말을 들으면 거창하게 느껴진다. 회사를 그만두고, 건물주가 되고, 배당금만으로 여행을 다니는 모습을 떠올리기 쉽다. 하지만 현실적인 경제적 자유는 그런 장면보다 훨씬 작고 구체적인 곳에서 시작된다.
...
적금 개설도 포함이 되나요? &nbsp; 입/출금 계좌만 해당된다. 적금이나 예금 게좌는 하루에 여러개 개설하여도 가능하다.`}).add({id:23,href:"/posts/fixed-variable-loan-interest/",title:"고정금리와 변동금리 차이 쉽게 정리: 대출받을 때 어떤...",description:"고정금리와 변동금리의 뜻, 대출금리 계산 구조, 시장금리 상승기와 하락기 선택 기준, 혼합금리까지 초보자도 이해하기 쉽게 정리했다.",content:`대출을 알아보다 보면 가장 헷갈리는 말 중 하나가 고정금리와 변동금리다. 은행 앱에서는 금리 숫자만 크게 보이지만, 실제로 중요한 것은 그 금리가 앞으로 계속 같은지, 일정 주기마다 바뀌는지다.
...
주식을 손해 보고 팔아도 증권거래세를 내야 할까? &nbsp; 내야 한다. 증권거래세는 이익이 아니라 매도 거래금액에 붙는 세금이다. 손실을 보고 팔아도 매도금액을 기준으로 세금이 계산되므로 잦은 매매를 할수록 거래비용이 누적될 수 있다.`}).add({id:27,href:"/posts/gross-domestic-product-gdp/",title:"국내총생산 GDP 뜻 쉽게 정리, 명목GDP와 실질GD...",description:"국내총생산 GDP의 뜻과 계산 개념, 명목GDP와 실질GDP의 차이, 1인당 GDP와 GNI까지 초보자도 이해하기 쉽게 정리했다.",content:`경제 뉴스를 보다 보면 GDP라는 단어가 정말 자주 나온다. &ldquo;한국 GDP 성장률&rdquo;, &ldquo;1인당 GDP&rdquo;, &ldquo;명목 GDP&rdquo;, &ldquo;실질 GDP&rdquo; 같은 표현이 계속 등장한다.
...
개인 투자자는 그레이 캐피털리즘을 왜 신경 써야 할까? &nbsp; 회색 지대 전략으로 이익을 내는 기업은 단기 실적이 좋아 보일 수 있다. 하지만 소비자 신뢰 하락, 규제 강화, 소송, 불매운동 같은 리스크가 생기면 기업가치가 흔들릴 수 있다. 투자자는 숫자만 보지 말고 그 숫자가 만들어지는 방식도 함께 봐야 한다.`}).add({id:30,href:"/posts/employment-income-tax-vs-global-income-tax/",title:"근로소득세와 종합소득세 차이, 직장인과 사업자는 세금 ...",description:"근로소득세와 종합소득세의 차이, 직장인 연말정산과 사업자 5월 신고, 부업 소득이 있는 직장인이 종합소득세를 신고해야 하는 경우를 쉽게 정리했다.",content:`나는 직장인이라 월급에서 근로소득세가 빠진다. 그런데 사업을 하는 친구는 매년 5월에 종합소득세를 신고한다고 했다. 둘 다 소득에 붙는 세금인 것 같은데, 왜 이름도 다르고 신고 방식도 다를까.
...
Q3. 이거 하나로 충분할까? &nbsp; 기초는 충분하다. 이후에는 뉴스나 투자 서적으로 확장하면 된다.`}).add({id:32,href:"/posts/financial-intelligence/",title:"금융 지능이란?",description:`금융 지능(Financial Intelligence) 은 쉽게 말해서 돈의 흐름을 이해하고, 합리적으로 돈을 관리하고, 장기적으로 자산을 늘릴 수 있는 능력을 의미한다.
...
금융 지식 감정 통제 실제 경험 결론적으로 금융 지능은 금융 지식, 합리적 판단 능력, 그리고 실제 재무 관리 경험이 결합된 개념이며, 개인의 경제적 안정과 자산 형성에 핵심적인 역할을 수행한다.`}).add({id:33,href:"/posts/bond-investing-when-rates-fall/",title:"기준금리가 내려가면 채권 가격은 왜 오를까? 금리 하락...",description:"기준금리와 채권 가격의 반비례 관계, 금리 하락기에 채권 투자가 주목받는 이유, 직접 채권과 채권 ETF 선택 기준을 초보자도 이해하기 쉽게 정리했다.",content:`채권 투자를 처음 공부하면 가장 자주 듣는 말이 있다. &ldquo;금리가 내려가면 채권 가격은 오른다&quot;는 말이다. 말 자체는 간단한데, 막상 왜 그런지 설명하려면 조금 헷갈린다. 예금 금리가 내려가면 이자를 덜 받는 것 같은데, 채권 가격은 왜 올라간다는 걸까.
...
예금금리보다 대출금리가 더 높은 이유는 무엇일까? &nbsp; 은행은 예금으로 돈을 조달하고 대출로 돈을 운용한다. 대출금리에는 은행의 조달비용뿐 아니라 운영비, 신용위험, 가산금리 등이 반영된다. 그래서 일반적으로 대출금리는 예금금리보다 높게 형성된다.`}).add({id:35,href:"/posts/multi-homeowner-capital-gains-tax-surcharge/",title:"다주택자 양도소득세 중과 5월 10일부터 시행: 대상,...",description:"2026년 5월 10일부터 다시 적용될 다주택자 양도소득세 중과의 뜻, 대상, 세율, 장기보유특별공제 배제, 매도 전 체크할 내용을 쉽게 정리했다.",content:`집을 여러 채 갖고 있는 사람에게 가장 민감한 세금 중 하나가 양도소득세다. 집을 보유하는 동안에는 재산세나 종부세가 부담이고, 집을 팔 때는 양도소득세가 부담이 된다. 특히 다주택자가 조정대상지역 주택을 팔 때 적용되는 양도세 중과 는 세금 차이가 꽤 크게 벌어질 수 있다.
...
Q3. 복리 효과는 언제부터 크게 느껴지나? &nbsp; 기간이 길어질수록 차이가 커진다. 짧게 보면 거의 비슷하지만, 5년, 10년, 20년처럼 장기로 갈수록 복리의 장점이 확실해진다.`}).add({id:37,href:"/posts/loan-ltv-dti-dsr/",title:"대출 받을 때 알아야 하는 LTV, DTI, DSR",description:" ",content:`LTV, DTI, DSR은 금융기관이 주택담보대출 심사 시 차주(借主, 돈을 빌리는 사람)의 상환 능력과 담보 가치를 평가하기 위해 사용하는 대표적인 규제 지표이다. 즉, 은행이 &ldquo;이 사람이 얼마나 대출을 감당할 수 있는지&rdquo; 판단하는 핵심 지표라고 할 수 있다.
...
사람들이 자주 오해하는 점 &nbsp; LTV 70%면 무조건 70% 대출 가능 → DSR에서 막히면 불가능 소득 높으면 대출 무조건 많이 가능 → LTV 제한 있음 한 줄 정리 &nbsp; LTV = 집 기준 DTI = 주담대 중심 소득 기준 DSR = 전체 빚 기준 (현재 핵심)`}).add({id:38,href:"/posts/loan-repayment-methods/",title:"대출 상환 방법 쉽게 정리, 원리금 균등·원금 균등·만...",description:"대출 상환 방법은 매달 갚는 금액과 총이자 비용을 크게 바꾼다. 원리금 균등, 원금 균등, 만기 일시, 거치 후 상환의 차이를 쉽게 정리한다.",content:`대출을 받을 때 금리만 보는 경우가 많다. 물론 금리는 중요하다. 그런데 실제로 매달 내 통장에서 빠져나가는 돈은 &ldquo;상환 방법&quot;에 따라 크게 달라진다.
...
대출을 빨리 갚고 싶다면 어떤 순서가 좋을까? &nbsp; 일반적으로 금리가 높은 대출부터 먼저 갚는 것이 효율적이다. 다만 중도상환수수료가 있는 상품도 있으므로 실제로 갚기 전에는 수수료와 절감되는 이자를 함께 계산해야 한다.`}).add({id:39,href:"/posts/deposit-loan-interest-basics/",title:"돈을 맡기면 왜 이자를 줄까? 예금금리와 대출금리의 원리",description:"은행에 돈을 맡기면 왜 이자를 주는지, 예금금리와 대출금리는 어떻게 다른지, 은행이 돈을 버는 예대마진 구조까지 초보자 눈높이로 정리했다.",content:`은행에 돈을 맡기면 시간이 지난 뒤 이자가 붙는다. 처음 금융을 배울 때는 이 부분이 조금 이상하게 느껴질 수 있다. 은행이 내 돈을 보관해주는데 왜 오히려 나에게 돈을 더 줄까.
...
정기보험 보험 기간이 정해져 있는 보험이다. 대부분 해약해도 해약환급금이 없거나 매우 적은 소멸성 보험으로 보험료가 저렴하고 가입하기 쉽다는 특징이 있다. 양로보험 정기보험과 마찬가지로 보험기간이 정해져 있으며, 보험기간 중 사망하면 사망보험금을, 만기까지 생존해 있으면 만기보험금을 받을 수 있다. 사망보험금과 만기보험금이 동일한 금액으로 &lsquo;생사혼합형 보험&rsquo;이라고도 한다. 종신보험 피보험자가 사망할 때까지 보장이 지속되는 보험이다. 사망 시점에 사망보험금이 지급된다. 보험료는 정기보험에 비해 비싸고, 보험기간 중 중도해지 시 해약환급금이 지급된다. 보험료는 정기보험에 비해 비싸다.`}).add({id:44,href:"/posts/value-added-tax-basics/",title:"부가가치세란 어떤 세금일까, 부가세 별도와 계산 방법 ...",description:"부가가치세의 뜻과 부가세 별도 의미, 최종소비자와 사업자의 관계, 매출세액과 매입세액을 이용한 계산 방법과 신고 기간을 쉽게 정리했다.",content:`물건을 사거나 서비스를 이용할 때 &ldquo;부가세 포함&rdquo;, &ldquo;부가세 별도&quot;라는 말을 자주 듣는다. 특히 견적서를 받을 때 100만 원이라고 생각했는데 마지막에 부가세 10만 원이 붙어 110만 원이 되는 경우가 있다. 그냥 내라고 하니까 내기는 하지만, 이 돈이 정확히 어떤 세금인지 헷갈릴 수 있다.
...
양성화 가능하다고 하면 매수해도 될까? &nbsp; 양성화 가능하다는 말만으로 매수하면 위험하다. 관할 구청에서 해당 건축물이 실제로 양성화 대상인지, 어떤 비용과 절차가 필요한지 확인해야 한다. 가능성이 아니라 확인된 사실을 기준으로 판단해야 한다.`}).add({id:48,href:"/posts/samsung-securities-isa-event/",title:"삼성증권 중개형 ISA 개설 이벤트 혜택 정리: 신규·...",description:"삼성증권 중개형 ISA 이벤트의 기간, 신규 개설 혜택, 타사 ISA 이전 금액 2배 인정, 국내주식 첫 거래 혜택과 주의사항을 쉽게 정리했다.",content:`요즘 ISA 계좌 하나쯤은 다들 고민한다. 절세에 좋다는 건 알겠는데, 막상 만들려고 하면 증권사도 많고 이벤트도 자주 바뀌어서 언제 개설해야 할지 애매하다.
...
신용대출을 빨리 갚으면 좋은가? &nbsp; 이자 부담을 줄일 수 있다는 점에서는 좋다. 다만 건별 신용대출은 상품에 따라 중도상환수수료가 있을 수 있으므로, 조기 상환 전에 수수료와 절감되는 이자를 함께 비교해야 한다.`}).add({id:52,href:"/posts/medical-expense-insurance-non-covered-items/",title:"실비보험 비급여 보장 총정리, 급여와 비급여 차이부터 ...",description:"실비보험에서 헷갈리는 급여와 비급여 차이, 비급여 보장 항목, 자기부담금, 도수치료·MRI·수액 청구 시 주의할 점을 누구나 이해하기 쉽게 정리하였다.",content:`병원비 영수증을 보면 급여, 비급여라는 단어가 나온다. 처음에는 그냥 병원에서 쓰는 어려운 말처럼 보이지만, 실비보험을 청구할 때는 이 두 단어를 반드시 알아야 한다. 내가 낸 병원비 중 어디까지 건강보험이 적용됐는지, 어디까지 실비보험으로 받을 수 있는지가 여기서 갈리기 때문이다.
...
비급여 진료를 받으면 어떤 서류를 챙겨야 할까? &nbsp; 기본적으로 진료비 영수증과 진료비 세부내역서를 챙기는 것이 좋다. 항목에 따라 처방전, 진단서, 의사 소견서, 통원확인서가 추가로 필요할 수 있으므로 보험사 앱의 청구 안내를 함께 확인해야 한다.`}).add({id:53,href:"/posts/medical-expense-insurance-claim-documents/",title:"실비보험 청구서류 정리, 통원·입원·약제비 청구 방법까...",description:"실비보험의 뜻과 통원·입원·약제비 청구서류, 진료비 영수증과 세부내역서 차이, 청구기간과 실손24 활용법을 누구나 이해하기 쉽게 정리하였다.",content:`살면서 아프지 않은 사람은 거의 없다. 가벼운 감기처럼 병원비가 몇천 원으로 끝날 때도 있지만, 검사나 치료가 이어지면 갑자기 몇십만 원이 나가기도 한다. 여유 자금이 있으면 버틸 수 있지만, 예상하지 못한 병원비는 누구에게나 부담이 된다.
...
주가가 떨어지면 무조건 버텨야 할까? &nbsp; 무조건 버티는 것은 장기투자가 아니다. 회사의 실적과 경쟁력이 유지되는데 시장 전체 하락으로 빠진 것인지, 기업 자체의 문제가 생긴 것인지 확인해야 한다. 처음 매수한 이유가 깨졌다면 매도도 검토해야 한다.`}).add({id:56,href:"/posts/pension-savings-irp-tax-credit/",title:"연금저축·IRP 세액공제 총정리: 한도, 환급액, IS...",description:"연금저축과 IRP 세액공제 한도, 소득별 공제율, 최대 환급액, ISA 만기자금 전환까지 연말정산 전에 알아둘 내용을 초보자 눈높이로 정리했다.",content:`연말정산을 할 때마다 &ldquo;이번에는 돌려받을까, 더 낼까&rdquo; 하는 생각이 먼저 든다. 연말정산은 1년 동안 미리 낸 세금과 실제로 부담해야 할 세금을 다시 맞추는 과정이다. 미리 낸 세금이 많으면 환급을 받고, 적게 냈으면 추가로 납부한다.
...
미국 직원은 팁을 못 받으면 최저임금도 못 받는 걸까? &nbsp; 연방 기준으로는 팁과 현금 임금을 합친 금액이 최저임금에 미치지 못하면 고용주가 차액을 보전해야 한다. 다만 실제 현장에서는 팁이 직원 소득에서 큰 비중을 차지하고, 주마다 임금 기준도 다르기 때문에 팁이 생계와 깊게 연결되어 있다.`}).add({id:58,href:"/posts/withholding-tax-system/",title:"원천징수 제도 뜻 쉽게 정리, 세전·세후 월급 차이까지...",description:"원천징수 제도의 뜻과 원천징수의무자, 세전·세후 월급 차이, 근로소득세가 미리 빠지는 이유, 연말정산과의 관계를 누구나 이해하기 쉽게 정리하였다.",content:`월급을 받으면 세전, 세후라는 말을 자주 듣는다. 연봉 계약서에는 분명 5,000만 원이라고 적혀 있는데, 실제 통장에 들어오는 돈을 계산해보면 생각보다 적다. 대략 &ldquo;세금 떼고 들어오는 돈이구나&rdquo; 정도는 알지만, 정확히 무엇이 빠지는지는 헷갈린다.
...
원천징수하면 세금 신고를 안 해도 될까? &nbsp; 직장인은 보통 회사의 연말정산으로 대부분 정리되지만, 다른 소득이 있거나 프리랜서·사업소득이 있는 경우 종합소득세 신고가 필요할 수 있다. 원천징수는 최종 확정이 아니라 미리 낸 세금인 경우가 많다.`}).add({id:59,href:"/posts/bank-vs-savings-bank/",title:"은행과 저축은행의 차이",description:`은행은 제1금융권이라고 하고, 이를 제외한 대부분의 금융기관을 제2금융권이라고 한다.
...
아무나 전입세대확인서를 발급받을 수 있을까? &nbsp; 아니다. 개인정보가 포함된 서류라서 소유자, 임차인, 매매계약자, 임대차계약자, 금융기관 등 법령상 신청 자격이 있는 사람이나 기관이 신청할 수 있다. 신청 유형에 따라 계약서, 위임장, 신분증 같은 서류가 필요할 수 있다.`}).add({id:64,href:"/posts/stock-leverage/",title:"주식 레버리지란 무엇인가? 초보자를 위한 쉬운 설명과 ...",description:"주식 레버리지는 적은 돈으로 더 큰 금액을 투자하는 방식이다. 수익도 커질 수 있지만 손실과 이자, 강제 청산 위험도 함께 커진다.",content:`주식 투자를 하다 보면 &ldquo;레버리지&quot;라는 말을 자주 듣게 된다. 특히 레버리지 ETF, 신용거래, 미수거래, 선물 같은 단어와 함께 등장한다.
...
공매도와 무차입 공매도는 같은 말일까? &nbsp; 다르다. 일반적인 공매도는 주식을 빌린 뒤 파는 차입 공매도다. 반면 무차입 공매도는 주식을 빌리지 않고 먼저 파는 방식으로, 국내에서는 원칙적으로 금지되어 있다. 투자자가 문제 삼는 경우도 대부분 불법적인 무차입 공매도나 제도 운영의 불공정성에 대한 우려와 관련이 있다.`}).add({id:69,href:"/posts/blue-chip-yellow-chip-red-chip/",title:"주식 용어: 블루칩 옐로우칩 레드칩 뜻과 차이 쉽게 이해하기",description:"주식시장에서 자주 쓰이는 블루칩, 옐로우칩, 레드칩의 뜻과 차이를 초보 투자자도 이해할 수 있게 사례와 함께 정리했다.",content:`주식 뉴스를 보다 보면 &ldquo;블루칩 중심으로 매수세가 들어왔다&rdquo;, &ldquo;옐로우칩이 강세를 보였다&rdquo;, &ldquo;홍콩 증시의 레드칩이 주목받았다&rdquo; 같은 표현을 만날 때가 있다. 처음 들으면 색깔로 주식을 나누는 것처럼 느껴지지만, 실제로는 기업의 규모, 안정성, 시장에서의 위치를 설명하는 용어다.
...
주택담보대출은 고정금리와 변동금리 중 무엇이 좋을까? &nbsp; 정답은 없다. 금리 상승이 걱정되고 월 상환액을 안정적으로 관리하고 싶다면 고정금리나 혼합형이 편할 수 있다. 반대로 금리 하락 가능성을 보고 초기 금리를 낮추고 싶다면 변동금리를 검토할 수 있다. 다만 변동금리는 금리가 오를 때 상환 부담이 커질 수 있다.`}).add({id:75,href:"/posts/house-type/",title:"주택의 종류",description:"주택 종류에 대해서 알아본다.",content:`주택이란? &nbsp; 주택은 주택법 제2조(정의)에 따라서 단독주택과 공동주택으로 구분된다.
...
&ldquo;월급에 모든 수당 포함&quot;이라고 쓰면 충분할까? &nbsp; 충분하지 않다. 기본급, 포함되는 수당 종류, 포함 시간, 수당 금액, 초과 시 별도 지급 기준을 구체적으로 적어야 한다. 그래야 실제 근로시간 기준 법정수당과 비교할 수 있다.`}).add({id:85,href:"/posts/foreign-stock-capital-gains-tax/",title:"해외주식 양도소득세 신고 방법, 250만 원 공제와 세...",description:"해외주식 양도소득세 신고 대상, 국내주식과 다른 과세 방식, 기본공제 250만 원, 세율 22%, 홈택스 신고 방법과 가산세를 쉽게 정리했다.",content:`해외주식을 하다 보면 매수와 매도 타이밍에는 민감해도 세금은 뒤로 미루기 쉽다. 특히 테슬라, 엔비디아, 애플, 마이크로소프트처럼 익숙한 미국 주식으로 수익이 났다면 &ldquo;증권사 계좌에 돈이 들어왔으니 끝난 것 아닌가&quot;라고 생각할 수 있다.
