...
환헤지 ETF와 환노출 ETF는 무엇이 다를까? &nbsp; 환헤지 ETF는 환율 변동 영향을 줄이고 기초 자산 수익률에 더 집중하도록 설계된 상품이고, 환노출 ETF는 환율 변동이 수익률에 반영되는 상품이다. 환율 하락이 걱정되면 환헤지가 유리할 수 있지만, 환헤지 비용이 생길 수 있다.`}).add({id:145,href:"/posts/foreign-stock-capital-gains-tax/",title:"해외주식 양도소득세 신고 방법, 250만 원 공제와 세...",description:"해외주식 양도소득세 신고 대상, 국내주식과 다른 과세 방식, 기본공제 250만 원, 세율 22%, 홈택스 신고 방법과 가산세를 쉽게 정리했다.",content:`해외주식을 하다 보면 매수와 매도 타이밍에는 민감해도 세금은 뒤로 미루기 쉽다. 특히 테슬라, 엔비디아, 애플, 마이크로소프트처럼 익숙한 미국 주식으로 수익이 났다면 &ldquo;증권사 계좌에 돈이 들어왔으니 끝난 것 아닌가&quot;라고 생각할 수 있다.
...
누수는 화재보험으로 보장될까? &nbsp; 상품과 특약에 따라 가능하다. 다만 기본 화재보험에 자동 포함되지 않을 수 있고, 노후 배관 교체비처럼 보장되지 않는 항목도 있다. 가입 전 누수 특약이 우리 집 손해와 아랫집 배상까지 포함하는지 확인해야 한다.`}).add({id:147,href:"/posts/etf-hedged-unhedged/",title:"환헤지 환노출 ETF 차이",description:"환헤지와 환노출 ETF의 뜻, 상품명 (H) 표시, 환율이 오르거나 내릴 때 수익률이 어떻게 달라지는지 초보자 기준으로 쉽게 정리했다.",content:`해외 주식 ETF를 고르다 보면 상품명 뒤에 (H)가 붙은 것을 자주 보게 된다. 예를 들어 &ldquo;미국S&amp;P500(H)&ldquo;처럼 표시된 상품이다. 처음 보면 단순한 상품 코드처럼 보이지만, 실제로는 환율 변동을 얼마나 반영할지 알려주는 중요한 표시다.
