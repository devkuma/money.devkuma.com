...
공매도와 무차입 공매도는 같은 말일까? &nbsp; 다르다. 일반적인 공매도는 주식을 빌린 뒤 파는 차입 공매도다. 반면 무차입 공매도는 주식을 빌리지 않고 먼저 파는 방식으로, 국내에서는 원칙적으로 금지되어 있다. 투자자가 문제 삼는 경우도 대부분 불법적인 무차입 공매도나 제도 운영의 불공정성에 대한 우려와 관련이 있다.`}).add({id:72,href:"/posts/blue-chip-yellow-chip-red-chip/",title:"주식 용어: 블루칩 옐로우칩 레드칩 뜻과 차이 쉽게 이해하기",description:"주식시장에서 자주 쓰이는 블루칩, 옐로우칩, 레드칩의 뜻과 차이를 초보 투자자도 이해할 수 있게 사례와 함께 정리했다.",content:`주식 뉴스를 보다 보면 &ldquo;블루칩 중심으로 매수세가 들어왔다&rdquo;, &ldquo;옐로우칩이 강세를 보였다&rdquo;, &ldquo;홍콩 증시의 레드칩이 주목받았다&rdquo; 같은 표현을 만날 때가 있다. 처음 들으면 색깔로 주식을 나누는 것처럼 느껴지지만, 실제로는 기업의 규모, 안정성, 시장에서의 위치를 설명하는 용어다.
...
주택담보대출은 고정금리와 변동금리 중 무엇이 좋을까? &nbsp; 정답은 없다. 금리 상승이 걱정되고 월 상환액을 안정적으로 관리하고 싶다면 고정금리나 혼합형이 편할 수 있다. 반대로 금리 하락 가능성을 보고 초기 금리를 낮추고 싶다면 변동금리를 검토할 수 있다. 다만 변동금리는 금리가 오를 때 상환 부담이 커질 수 있다.`}).add({id:78,href:"/posts/house-type/",title:"주택의 종류",description:"주택 종류에 대해서 알아본다.",content:`주택이란? &nbsp; 주택은 주택법 제2조(정의)에 따라서 단독주택과 공동주택으로 구분된다.
...
완제 &nbsp; 대출 받은 금액을 전액 상환하면 대출을 완제하였다고 한다.`}).add({id:83,href:"/posts/carry-trade/",title:"캐리 트레이드란? 금리 차이로 수익을 노리는 외환 투자...",description:"캐리 트레이드의 뜻과 작동 원리, 금리 차익과 환율 변동 수익, 장점과 위험, 개인 투자자가 주의할 점을 쉽게 정리했다.",content:`환율 뉴스를 보다 보면 &ldquo;엔 캐리 트레이드 청산&rdquo;, &ldquo;달러 강세와 캐리 트레이드&rdquo;, &ldquo;고금리 통화로 자금이 이동한다&rdquo; 같은 표현이 나온다. 주식이나 예금보다 낯선 말이라 어렵게 느껴지지만, 핵심은 단순하다. 금리가 낮은 통화로 돈을 조달하고, 금리가 높은 통화에 투자해 차이를 노리는 전략이다.
...
엔 캐리 트레이드는 무엇인가? &nbsp; 엔 캐리 트레이드는 금리가 낮은 엔화로 자금을 조달해 상대적으로 금리가 높은 통화나 자산에 투자하는 전략을 말한다. 일본의 저금리 환경 때문에 오랫동안 대표적인 캐리 트레이드 사례로 언급되었다. 다만 일본 금리와 글로벌 위험 심리가 바뀌면 엔 캐리 트레이드도 빠르게 청산될 수 있다.`}).add({id:84,href:"/posts/kospi-kosdaq-nasdaq/",title:"코스피, 코스닥, 나스닥 간단 설명",description:"주식 투자에 관심이 있다면 반드시 구분해서 이해해야 할 지수가 바로 코스피, 코스닥, 나스닥이다. 각각의 개념과 차이점에 설명한다.",content:`코스피 (KOSPI) &nbsp; 정식 명칭: Korea Composite Stock Price Index 운영 기관: 한국거래소 개념 &nbsp; 대한민국 유가증권시장(대형 우량기업 시장) 에 상장된 기업들의 주가를 종합해 만든 대표 지수이다.
...
&ldquo;월급에 모든 수당 포함&quot;이라고 쓰면 충분할까? &nbsp; 충분하지 않다. 기본급, 포함되는 수당 종류, 포함 시간, 수당 금액, 초과 시 별도 지급 기준을 구체적으로 적어야 한다. 그래야 실제 근로시간 기준 법정수당과 비교할 수 있다.`}).add({id:89,href:"/posts/foreign-stock-capital-gains-tax/",title:"해외주식 양도소득세 신고 방법, 250만 원 공제와 세...",description:"해외주식 양도소득세 신고 대상, 국내주식과 다른 과세 방식, 기본공제 250만 원, 세율 22%, 홈택스 신고 방법과 가산세를 쉽게 정리했다.",content:`해외주식을 하다 보면 매수와 매도 타이밍에는 민감해도 세금은 뒤로 미루기 쉽다. 특히 테슬라, 엔비디아, 애플, 마이크로소프트처럼 익숙한 미국 주식으로 수익이 났다면 &ldquo;증권사 계좌에 돈이 들어왔으니 끝난 것 아닌가&quot;라고 생각할 수 있다.
