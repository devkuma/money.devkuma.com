...
처음 사는 S&amp;P500 ETF는 몇 주부터 시작하면 좋을까? &nbsp; 처음이라면 1주로 충분하다. 중요한 것은 큰돈을 넣는 것이 아니라 매수, 체결, 평가손익, 분배금, 세금 안내를 직접 확인하는 경험이다. 1주를 사보고 상품 구조를 이해한 뒤에 적립식 투자 여부를 정해도 늦지 않다.`}).add({id:21,href:"/posts/sp500-vs-schd-which-to-buy/",title:"S&P500 vs SCHD, 뭐 사야 할까? 수익률부터...",description:"S&P500과 SCHD 중 무엇을 사야 할지 고민이라면 이 글 하나로 끝내자. 수익률, 배당, 투자 성향별 추천까지 초보자 기준으로 쉽게 정리했다.",content:`S&amp;P500 vs SCHD, 처음 투자 시작할 때 거의 무조건 마주치는 고민이다.
...
Q3. 둘 중 하나만 꼭 골라야 한다면? &nbsp; 초보자 기준에서는 S&amp;P500을 먼저 선택하는 게 더 합리적이다. 이후 상황에 따라 SCHD를 추가하면 된다.`}).add({id:22,href:"/posts/tqqq-qld-qqq-nasdaq-etf/",title:"TQQQ QLD QQQ 뜻, 나스닥 100 추종 ETF...",description:"QQQ, QLD, TQQQ의 뜻과 차이를 나스닥 100 ETF 구조, 레버리지 배율, 장단점, 초보 투자자가 조심해야 할 부분까지 쉽게 정리했다.",content:`미국 기술주 투자를 이야기할 때 나스닥 100을 빼놓기 어렵다. 애플, 마이크로소프트, 엔비디아, 아마존, 테슬라처럼 전 세계 투자자가 관심을 갖는 기업들이 많이 들어 있기 때문이다. 그래서 미국 주식에 직접 투자하지 않더라도 &ldquo;나스닥 ETF 하나쯤은 사야 하나?&ldquo;라는 고민을 하게 된다.
...
레버리지 ETF는 왜 횡보장에 불리할 수 있나? &nbsp; 레버리지 ETF는 매일 목표 배율을 맞추는 구조다. 가격이 오르내리는 과정에서 손실과 회복이 반복되면 단순 배율 계산과 다른 결과가 나온다. 특히 변동성이 큰 횡보장에서는 기초지수가 제자리여도 레버리지 ETF 수익률은 나빠질 수 있다.`}).add({id:23,href:"/posts/vix-index/",title:"VIX 지수란? 공포지수로 보는 주식시장 변동성과 투자...",description:"VIX 지수의 뜻과 계산 원리, S&P500과의 관계, 투자자가 활용하는 방법, VIX 선물과 ETF 투자 시 주의할 점을 쉽게 정리했다.",content:`주식시장이 크게 흔들릴 때 뉴스에서 자주 나오는 말이 있다. 바로 VIX 지수다. &ldquo;공포지수가 급등했다&rdquo;, &ldquo;VIX가 안정되면서 증시가 반등했다&rdquo; 같은 표현을 한 번쯤 봤을 것이다.
...
VIX ETF를 장기 보유해도 될까? &nbsp; 주의가 필요하다. VIX ETF나 ETN은 대부분 VIX 선물을 활용하기 때문에 장기 보유 수익률이 VIX 지수와 다르게 움직일 수 있다. 선물 만기 교체 비용과 큰 변동성 때문에 단기 대응 상품에 가까운 경우가 많다.`}).add({id:24,href:"/posts/sole-proprietor-vs-corporation/",title:"개인사업자와 법인사업자 차이, 창업 전에 꼭 알아야 할 기준",description:"창업할 때 개인사업자와 법인사업자 중 무엇을 선택해야 할지 설립 절차, 세금, 책임 범위, 자금 조달, 운영 방식 차이로 쉽게 정리했다.",content:`창업을 준비하다 보면 생각보다 빨리 마주치는 질문이 있다. &ldquo;개인사업자로 시작할까, 법인사업자로 시작할까?&ldquo;라는 문제다. 아이템을 정하고, 상호를 고민하고, 사업장을 알아보는 일도 중요하지만 사업 형태를 어떻게 정하느냐에 따라 세금, 책임, 회계 처리, 투자 유치 방식이 달라진다.
...
경기지역화폐 카드를 받기 전에도 충전할 수 있을까? &nbsp; 지역과 카드 상태에 따라 앱에서 충전 메뉴가 보일 수 있지만, 실제 결제는 카드 등록 이후 가능한 경우가 많다. 처음 신청했다면 카드를 수령한 뒤 앱에 등록하고 계좌 연결까지 마친 다음 충전하는 흐름으로 보는 것이 안전하다.`}).add({id:27,href:"/posts/how-to-study-economics/",title:"경제 공부하는 방법, 금융 문맹을 벗어나기 위한 현실적...",description:"경제 공부를 어디서부터 시작해야 할지 모르는 초보자를 위해 기본 용어, 경제 뉴스 읽기, 책 선택, 가계부와 소액 투자까지 현실적인 공부 방법을 정리했다.",content:`경제 공부를 해야겠다고 마음먹는 순간은 대개 불편함에서 온다. 월급은 들어오는데 돈이 어디로 나가는지 모르겠고, 금리가 오른다는데 내 대출이 왜 부담스러워지는지 모르겠고, 뉴스에서는 환율과 물가 이야기가 계속 나오는데 내 생활과 어떻게 연결되는지 감이 잡히지 않는다.
...
위기 때 주식은 모두 팔아야 할까? &nbsp; 모든 주식을 감정적으로 매도하는 것은 위험할 수 있다. 당장 쓸 돈으로 투자했다면 줄이는 것이 맞지만, 장기 자금이라면 비상금과 부채 상황을 먼저 확인한 뒤 분할 매수나 리밸런싱 기준에 따라 판단하는 것이 낫다.`}).add({id:30,href:"/posts/spillover-effect/",title:"경제용어: 스필오버 효과, 경제에서 좋은 영향과 나쁜 ...",description:"스필오버 효과의 뜻과 경제, 관광, 조직관리, 상권에서 나타나는 긍정적 영향과 부정적 영향을 초보자도 이해하기 쉽게 정리했다.",content:`경제 기사를 읽다 보면 &ldquo;스필오버 효과&quot;라는 표현이 종종 나온다. 영어로는 Spillover Effect라고 하고, 말 그대로 어떤 영향이 한곳에만 머무르지 않고 주변으로 흘러넘치는 현상을 뜻한다.
...
투자할 때도 스필오버 효과를 볼 수 있을까? &nbsp; 볼 수 있다. 특정 산업의 대규모 투자가 장비, 소재, 부품, 물류 기업으로 번질 수 있고, 한 나라의 금리 변화가 다른 나라 주식시장과 환율에 영향을 줄 수도 있다. 투자자는 직접 수혜주뿐 아니라 주변으로 번지는 영향을 함께 살펴볼 필요가 있다.`}).add({id:31,href:"/posts/financial-freedom-roadmap/",title:"경제적 자유, 누구나 이룰 수 있는 실천 로드맵",description:"경제적 자유를 막연한 꿈이 아니라 실천 가능한 목표로 만들기 위해 지출 관리, 비상금, 부채 정리, 투자, 부수입 로드맵을 정리했다.",content:`경제적 자유라는 말을 들으면 거창하게 느껴진다. 회사를 그만두고, 건물주가 되고, 배당금만으로 여행을 다니는 모습을 떠올리기 쉽다. 하지만 현실적인 경제적 자유는 그런 장면보다 훨씬 작고 구체적인 곳에서 시작된다.
...
적금 개설도 포함이 되나요? &nbsp; 입/출금 계좌만 해당된다. 적금이나 예금 게좌는 하루에 여러개 개설하여도 가능하다.`}).add({id:33,href:"/posts/fixed-variable-loan-interest/",title:"고정금리와 변동금리 차이 쉽게 정리: 대출받을 때 어떤...",description:"고정금리와 변동금리의 뜻, 대출금리 계산 구조, 시장금리 상승기와 하락기 선택 기준, 혼합금리까지 초보자도 이해하기 쉽게 정리했다.",content:`대출을 알아보다 보면 가장 헷갈리는 말 중 하나가 고정금리와 변동금리다. 은행 앱에서는 금리 숫자만 크게 보이지만, 실제로 중요한 것은 그 금리가 앞으로 계속 같은지, 일정 주기마다 바뀌는지다.
