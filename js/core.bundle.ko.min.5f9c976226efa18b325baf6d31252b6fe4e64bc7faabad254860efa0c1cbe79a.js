...
원천징수하면 세금 신고를 안 해도 될까? &nbsp; 직장인은 보통 회사의 연말정산으로 대부분 정리되지만, 다른 소득이 있거나 프리랜서·사업소득이 있는 경우 종합소득세 신고가 필요할 수 있다. 원천징수는 최종 확정이 아니라 미리 낸 세금인 경우가 많다.`}).add({id:70,href:"/posts/bank-vs-savings-bank/",title:"은행과 저축은행의 차이",description:`은행은 제1금융권이라고 하고, 이를 제외한 대부분의 금융기관을 제2금융권이라고 한다.
...
아무나 전입세대확인서를 발급받을 수 있을까? &nbsp; 아니다. 개인정보가 포함된 서류라서 소유자, 임차인, 매매계약자, 임대차계약자, 금융기관 등 법령상 신청 자격이 있는 사람이나 기관이 신청할 수 있다. 신청 유형에 따라 계약서, 위임장, 신분증 같은 서류가 필요할 수 있다.`}).add({id:76,href:"/posts/tax-saving-vs-tax-evasion/",title:"절세와 탈세 차이, 합법적으로 세금 줄이는 기준과 탈세 사례",description:"절세와 탈세의 차이를 세법상 기준, 증빙자료, 소득공제와 세액공제, 가공경비와 수입 누락 사례로 초보자도 이해하기 쉽게 정리했다.",content:`세금은 가능하면 줄이고 싶다. 월급을 받는 사람도 연말정산 환급액이 커지면 기분이 좋고, 사업자는 종합소득세나 부가가치세 신고 때 비용을 빠짐없이 인정받고 싶다. 그런데 여기서 자주 헷갈리는 말이 있다. 바로 &ldquo;절세&quot;와 &ldquo;탈세&quot;다.
...
해외 주식도 같은 계좌에서 살 수 있을까? &nbsp; 대부분의 증권사 종합매매계좌에서 해외 주식 거래를 신청하면 가능하다. 다만 해외 주식은 환전, 거래 시간, 세금, 수수료가 국내 주식과 다르다. 미국 주식을 살 생각이라면 해외 주식 거래 신청뿐 아니라 환율과 양도소득세도 함께 공부하는 것이 좋다.`}).add({id:82,href:"/posts/stock-deposit/",title:"주식 예수금 뜻: D+2 출금, 주문가능금액, 미수금까...",description:"주식 예수금의 뜻과 D+2 출금 구조, 주문가능금액과 출금가능금액의 차이, 미수금과 반대매매 위험까지 초보자 눈높이로 정리했다.",content:`주식을 처음 팔아보면 이상한 순간이 온다. 분명히 매도 체결은 됐는데 은행 계좌로 바로 돈을 빼려고 하면 출금 가능 금액이 늘어나지 않는다. 앱에는 예수금, D+1, D+2, 주문가능금액 같은 말이 같이 나오니 더 헷갈린다.
...
공매도와 무차입 공매도는 같은 말일까? &nbsp; 다르다. 일반적인 공매도는 주식을 빌린 뒤 파는 차입 공매도다. 반면 무차입 공매도는 주식을 빌리지 않고 먼저 파는 방식으로, 국내에서는 원칙적으로 금지되어 있다. 투자자가 문제 삼는 경우도 대부분 불법적인 무차입 공매도나 제도 운영의 불공정성에 대한 우려와 관련이 있다.`}).add({id:85,href:"/posts/blue-chip-yellow-chip-red-chip/",title:"주식 용어: 블루칩 옐로우칩 레드칩 뜻과 차이 쉽게 이해하기",description:"주식시장에서 자주 쓰이는 블루칩, 옐로우칩, 레드칩의 뜻과 차이를 초보 투자자도 이해할 수 있게 사례와 함께 정리했다.",content:`주식 뉴스를 보다 보면 &ldquo;블루칩 중심으로 매수세가 들어왔다&rdquo;, &ldquo;옐로우칩이 강세를 보였다&rdquo;, &ldquo;홍콩 증시의 레드칩이 주목받았다&rdquo; 같은 표현을 만날 때가 있다. 처음 들으면 색깔로 주식을 나누는 것처럼 느껴지지만, 실제로는 기업의 규모, 안정성, 시장에서의 위치를 설명하는 용어다.
...
주택담보대출은 고정금리와 변동금리 중 무엇이 좋을까? &nbsp; 정답은 없다. 금리 상승이 걱정되고 월 상환액을 안정적으로 관리하고 싶다면 고정금리나 혼합형이 편할 수 있다. 반대로 금리 하락 가능성을 보고 초기 금리를 낮추고 싶다면 변동금리를 검토할 수 있다. 다만 변동금리는 금리가 오를 때 상환 부담이 커질 수 있다.`}).add({id:91,href:"/posts/house-type/",title:"주택의 종류",description:"주택 종류에 대해서 알아본다.",content:`주택이란? &nbsp; 주택은 주택법 제2조(정의)에 따라서 단독주택과 공동주택으로 구분된다.
...
아파트 &nbsp; 주택으로 쓰는 층수가 5층 이상인 주택을 말한다.`}).add({id:92,href:"/posts/direct-vs-indirect-investing/",title:"직접투자와 간접투자 차이, 종류와 장단점 쉽게 정리",description:"직접투자와 간접투자의 뜻, 대표적인 종류, 각각의 장점과 단점, 초보자가 어떤 방식으로 시작하면 좋은지 현실적인 기준으로 정리하였다.",content:`투자를 시작하려고 하면 가장 먼저 부딪히는 고민이 있다. 내가 직접 종목을 고를 것인지, 아니면 펀드나 ETF처럼 누군가 운용하는 상품을 활용할 것인지다. 주변에서는 &ldquo;좋은 주식을 직접 사야 돈을 번다&quot;고 말하기도 하고, 또 다른 쪽에서는 &ldquo;초보자는 분산투자 상품부터 해야 한다&quot;고 말한다.
...
현실적인 방법은 둘 중 하나만 고르는 것이 아니라 섞는 것이다. 예를 들어 전체 투자금의 7080%는 시장 전체에 투자하는 ETF나 펀드로 두고, 나머지 2030%만 관심 있는 개별 주식에 직접투자하는 방식이다. 이렇게 하면 투자 공부도 하면서 전체 계좌가 한 종목에 휘둘리는 위험을 줄일 수 있다.
...
간접투자는 안전한 투자인가? &nbsp; 간접투자도 원금 손실이 날 수 있다. 펀드나 ETF가 여러 자산에 나눠 투자한다고 해서 손실이 사라지는 것은 아니다. 투자 대상, 보수, 환매 조건, 변동성을 확인해야 한다.`}).add({id:93,href:"/posts/rolling-effect/",title:"채권 용어: 롤링 효과, 채권 가격이 만기가 가까워질수...",description:"롤링 효과의 뜻과 채권 가격, 잔존기간, 수익률곡선의 관계를 초보자도 이해하기 쉽게 예시와 함께 정리했다.",content:`채권 투자를 공부하다 보면 &ldquo;롤링 효과&quot;라는 용어를 듣게 된다. 처음 들으면 어려운 전문 용어처럼 느껴지지만, 핵심은 단순하다. 시간이 지나면서 채권의 남은 만기가 짧아지고, 그 과정에서 채권 가격이 오를 수 있다는 뜻이다.
...
완제 &nbsp; 대출 받은 금액을 전액 상환하면 대출을 완제하였다고 한다.`}).add({id:97,href:"/posts/carry-trade/",title:"캐리 트레이드란? 금리 차이로 수익을 노리는 외환 투자...",description:"캐리 트레이드의 뜻과 작동 원리, 금리 차익과 환율 변동 수익, 장점과 위험, 개인 투자자가 주의할 점을 쉽게 정리했다.",content:`환율 뉴스를 보다 보면 &ldquo;엔 캐리 트레이드 청산&rdquo;, &ldquo;달러 강세와 캐리 트레이드&rdquo;, &ldquo;고금리 통화로 자금이 이동한다&rdquo; 같은 표현이 나온다. 주식이나 예금보다 낯선 말이라 어렵게 느껴지지만, 핵심은 단순하다. 금리가 낮은 통화로 돈을 조달하고, 금리가 높은 통화에 투자해 차이를 노리는 전략이다.
...
엔 캐리 트레이드는 무엇인가? &nbsp; 엔 캐리 트레이드는 금리가 낮은 엔화로 자금을 조달해 상대적으로 금리가 높은 통화나 자산에 투자하는 전략을 말한다. 일본의 저금리 환경 때문에 오랫동안 대표적인 캐리 트레이드 사례로 언급되었다. 다만 일본 금리와 글로벌 위험 심리가 바뀌면 엔 캐리 트레이드도 빠르게 청산될 수 있다.`}).add({id:98,href:"/posts/kospi-kosdaq-nasdaq/",title:"코스피, 코스닥, 나스닥 간단 설명",description:"주식 투자에 관심이 있다면 반드시 구분해서 이해해야 할 지수가 바로 코스피, 코스닥, 나스닥이다. 각각의 개념과 차이점에 설명한다.",content:`코스피 (KOSPI) &nbsp; 정식 명칭: Korea Composite Stock Price Index 운영 기관: 한국거래소 개념 &nbsp; 대한민국 유가증권시장(대형 우량기업 시장) 에 상장된 기업들의 주가를 종합해 만든 대표 지수이다.
...
콜옵션과 풋옵션은 초보자가 직접 투자해도 괜찮나? &nbsp; 옵션은 만기, 변동성, 시간 가치가 얽힌 파생상품이라 초보자가 바로 직접 거래하기에는 어렵다. 먼저 구조와 손익 계산을 이해하고, 옵션 전략이 들어간 금융상품을 볼 때 설명서를 읽는 용도로 접근하는 것이 현실적이다.`}).add({id:100,href:"/posts/land-transaction-permit-system/",title:"토지거래허가제란? 실거주 의무와 계약 전 체크 포인트 ...",description:"토지거래허가제가 무엇인지, 허가 대상과 실거주 의무, 허가 없는 계약의 효력, 계약 전 확인해야 할 서류와 위반 시 제재를 쉽게 정리했다.",content:`부동산 뉴스를 보다 보면 &ldquo;토지거래허가구역&rdquo;, &ldquo;토허제&rdquo;, &ldquo;실거주 2년&rdquo; 같은 말이 자주 나온다. 이름만 보면 땅을 사고팔 때만 해당되는 제도처럼 느껴지지만, 실제로는 아파트 매매에도 영향을 줄 수 있다. 특히 서울 전역이나 경기 일부 지역처럼 아파트가 토지거래허가구역으로 지정된 곳에서는 일반적인 매매계약처럼 생각하고 접근하면 낭패를 볼 수 있다.
...
허가를 못 받으면 계약금은 어떻게 되나? &nbsp; 허가가 거부되어 계약이 확정적으로 무효가 되면 이미 지급한 계약금이나 중도금은 부당이득 반환 문제로 정리될 수 있다. 다만 계약서 문구, 귀책사유, 허가 신청 협력 여부에 따라 분쟁이 달라질 수 있으므로 허가 대상 거래는 계약서에 불허가 시 처리 방법을 미리 적어두는 것이 좋다.`}).add({id:101,href:"/posts/retirement-pension-default-option/",title:"퇴직연금 디폴트옵션 뜻과 선택 방법, DC형 IRP 가...",description:"퇴직연금 디폴트옵션의 뜻, 적용 대상, 발동 방식, 위험등급별 포트폴리오와 TDF·BF·SVF·SOC 상품 차이를 초보자도 이해하기 쉽게 정리하였다.",content:`퇴직연금 계좌를 열어두고도 한 번도 운용 상품을 바꿔본 적이 없다면 디폴트옵션을 알아둘 필요가 있다. 퇴직연금은 오래 묵혀두는 돈이라 작은 수익률 차이도 시간이 지나면 꽤 크게 벌어진다.
...
&ldquo;월급에 모든 수당 포함&quot;이라고 쓰면 충분할까? &nbsp; 충분하지 않다. 기본급, 포함되는 수당 종류, 포함 시간, 수당 금액, 초과 시 별도 지급 기준을 구체적으로 적어야 한다. 그래야 실제 근로시간 기준 법정수당과 비교할 수 있다.`}).add({id:105,href:"/posts/foreign-stock-capital-gains-tax/",title:"해외주식 양도소득세 신고 방법, 250만 원 공제와 세...",description:"해외주식 양도소득세 신고 대상, 국내주식과 다른 과세 방식, 기본공제 250만 원, 세율 22%, 홈택스 신고 방법과 가산세를 쉽게 정리했다.",content:`해외주식을 하다 보면 매수와 매도 타이밍에는 민감해도 세금은 뒤로 미루기 쉽다. 특히 테슬라, 엔비디아, 애플, 마이크로소프트처럼 익숙한 미국 주식으로 수익이 났다면 &ldquo;증권사 계좌에 돈이 들어왔으니 끝난 것 아닌가&quot;라고 생각할 수 있다.
